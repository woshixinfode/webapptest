<template>
  <div>
    ...
    <group>
      ...
      <x-textarea placeholder="讲讲这张照片" v-model="uploadsummary" :show-counter="true" :rows="2" :max="1000"
                  autosize></x-textarea>
    </group>
  ...
  export default {
    ...
    methods: {
      ...
      uploadall() {
        ...
        if (this.limit == 1 && this.imgList.length > 1) {
          this.warningalert("只允许上传一张照片！")
          return
        }
      ...
      fileAdd(file) {
        ...
        console.log(this.limit == 1 && this.imgList.length > 1);
        if (this.limit == 1 && this.imgList.length > 0) {
          this.warningalert("只允许上传一张图片！")
          return
            ...
            .post(this.directUploadInfo.action, data, {
              onUploadProgress: progress => {
                this.$nextTick(() => {
                  ...
                  this.$set(this.imgListpost, arrayIndex, item);
                });
              }
            })
            .then(x => {
                ...
                EXIF.getData(item.file, function () {
                  ...
                  ims.onload = function () {
                    ...
                    saveUploadingAsset(data, {
                      ...
                      if (x.status_code == 1) {
                        item.status = "success";
                        _that.$set(_that.uploadFileListView, arrayIndex, item);
      ...
      ossUploadAction(item, index) {
        ...
        if ((item.status == "ready" && item.percentage == 0) || item.status == "error") {
          ...
          let filekey = `${this.directUploadInfo.dir}photo/${item.uid}.${getfileSuffix(item.file.name)}`;
          filekey = filekey.substring(1, filekey.length);
          //debugger;
          this.ossClient
            .multipartUpload(filekey, item.file, {
            ...
            .then(k => {
              if (k.res.status == 200) {
                EXIF.getData(item.raw, function () {
                  ...
                  var _iptcdata = this.iptcdata;
                  var ims = new Image();
                  ims.src = URL.createObjectURL(item.file);
                  ims.onload = function () {
                    var data = {
                      hid: `${item.uid}.${getfileSuffix(item.file.name)}`, //上传至对象存储时所生成的唯一对象文件名称 *
                      asset_type: "1", //1图片2音频3视频{支持的格式参考备注中图片资源支持格式详情} *
      ...
      startUpload() {
        ...
        if (this.directUploadInfo.storage == "aws") {
          console.log("aws")
          this.limit = this.$store.state.limit
          if (this.limit == 1 && this.imgListpost.length > 1) {
            this.warningalert("只允许上传一张图片！")
            return;
          }
          this.imgListpost.forEach((item, index) => {
            console.log(item, index)
            this.realUploadAction(item, index);
          });
          ...
          this.imgListpost.forEach((item, index) => {
            this.ossUploadAction(item, index);
          });
        }
      },
      ossInIt() {
